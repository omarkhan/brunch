(function() {
  var Compiler, colors, fs, helpers, options, path;
    ...
    StylusCompiler.prototype.compile = function(files) {
      ...
      return fs.readFile(main_file_path, 'utf8', __bind(function(err, data) {
        ...
        if (err != null) {
          ...
          compiler = stylus(data).set('filename', main_file_path).set('compress', true).include(path.join(options.brunchPath, 'src'));
          if (this.nib()) {
            compiler.use(this.nib());
          }
          return compiler.render(function(err, css) {
            if (err != null) {
              return helpers.log(colors.lred('stylus err: ' + err));
            } else {
              return fs.writeFile(path.join(options.buildPath, 'web/css/main.css'), css, 'utf8', __bind(function(err) {
                if (err != null) {
                  return helpers.log(colors.lred('stylus err: ' + err));
                } else {
